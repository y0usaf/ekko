...
impl Grid {
    ...
    /// The number of rows currently stored in the scrollback (as opposed
    /// to `scrollback_len`, which is the configured maximum).
    pub fn history_len(&self) -> usize {
        self.scrollback.len()
    }

    /// Iterate over the rows stored in the scrollback, oldest first.
