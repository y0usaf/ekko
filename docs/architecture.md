...
| `wire.lisp` | Version-16 routed IPC, input binding generations, bounded peer queues, scene acknowledgements, private daemon directory |
| `daemon.lisp` | The workspace: pane/view registries, reactor, startup pipeline, and shutdown |
...
Workspace creation is staged in the reactor, so the daemon answers peers while
configuration loads. The first request captures its cwd, environment and config
path; workers and subsequent panes use that launch context. Later `run` requests
open panes in the existing workspace with their own cwd/environment. Only child
processes change cwd/environment, never the daemon.
...
replacement reconstructs owned contributions while preserving workspace state.
...
state. Detach invalidates pending input and old-origin actions.
...
its generation, so a late old-view event cannot enter the new view.
...
remain. Every peer begins with a version-16 route naming an optional view.
Old versions and unversioned controls are rejected explicitly.
