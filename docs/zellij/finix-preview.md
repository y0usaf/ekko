...
fullscreen, and `c` renames. Ctrl-h enters Move mode. The full Zellij mode/bar/tab,
floating, plugin, session, CLI, and other surface requirements remain open.

The launch prints a private configuration path and the exact command to reload
...
copy-input, scrollback, and wire fixes. Only two overlapping geometry fixes were
removed from that patch because the parity source already contains them. The
