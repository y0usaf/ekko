    ...
    (dolist (span spans)
      ...
      (let ((keys nil))
        (loop for (key value) on span by #'cddr do
          (unless (member key '(:x :y :text :sgr :rows :overlay :action)) (error "Unknown decoration field: ~S" key))
        ...
        (let ((x (getf span :x)) (y (getf span :y)) (text (getf span :text))
          ...
          (let ((action (getf span :action)))
            (when action
              (unless (and (listp action) (= (length action) 3)
                           (member (first action) '(:focus :minimize :restore :zoom :close))
    ...
    (dolist (a actions)
      ...
      (let* ((op (first a)) (args (rest a)) (pane (getf args :pane))
             (keys (case op
                     ...
                     (:close '(:pane :focus)) ((:minimize :restore :zoom) '(:pane)) (:copy-move '(:delta)) (:copy-edge '(:edge)) (:status '(:text))
          ...
          (list :object :generation (session-config-generation session) :error (session-config-error session)
                ...
                :panes (loop for p in (session-panes session) for vt = (pane-vt p) collect
                         (list :object :id (pane-id p) :label (pane-label p)
                               ...
                               :minimized (if (pane-minimized p) t :false)
