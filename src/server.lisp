...
(defstruct pane id pid io vt (graphics (make-store)) argv label status (activation-order 0) (x 0) (y 0)
...
(defstruct session name panes (cols 120) (rows 36) (cw 8) (ch 16) (focus 0)
  ...
  key-fragment key-fragment-mode)
    ...
    (dolist (rect (let ((shown (session-rectangles session)))
      ...
      (destructuring-bind (id x y width height) rect
        ...
        (unless (equal (list cols rows (session-cw session) (session-ch session))
          ...
          (when (pane-io pane)
            (resize (wire-fd (pane-io pane)) cols rows (* cols (session-cw session)) (* rows (session-ch session)))))))))
  (incf (session-revision session)))
           ...
           (loop for argv in commands for pane in (session-panes session) do
             (let ((vt (pane-vt pane)))
               (multiple-value-bind (fd pid)
                   (spawn argv (terminal-cols vt) (terminal-rows vt)
                          (session-cw session) (session-ch session))
