              ...
              append
                ...
                (zellij-frame-spans
                 (list :rect (getf pane :outer-rect)
                       :title (if note (getf note :text) (or (getf pane :label) ""))
...
(ekko/extensions:register-component
 :id :zellij-modes :reads '(:mode :focus :panes :viewport :zoom))
(dolist (mode '(:normal :locked :pane))
  (ekko/extensions:register-keymap :component :zellij-modes :name mode
                                    :unbound (if (eq mode :pane) :ignore :forward)))
